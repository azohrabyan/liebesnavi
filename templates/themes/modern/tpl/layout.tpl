...
<head>
    ...
    {{ $design->staticFiles('css', PH7_STATIC . PH7_CSS . 'js/jquery/box/', 'box.css') }
    } {* We have to include box CSS alone because it includes images in its folder *}
    ...
    {if $is_admin_auth}
        <script>pH7Url.admin_mod = '{url_admin_mod}';</script>
    {/if}
...
<header>
    ...
    <noscript>
        <div class="noscript err_msg">
            {lang}JavaScript is disabled on your Web browser!
                <br/>
                Please enable JavaScript via the options of your Web browser in order to use this website.{/lang}
        </div>
    </noscript>
    ...
    <div class="main_message" id="main_message">
        <div class="main_message_title">Messagner
            <div class="main_message_close">
                <a onclick="document.getElementById('main_message').style.display='none'"><img
        ...
        <div class="main_message_chat_send" id="textarea_X">
            <textarea class="main_chatboxtextarea main_chatboxtextareaselected"
                      onkeydown="return Messenger.checkBoxInputKey(event,this,'');"></textarea>
        </div>
        <div class="main_message_info">
            <div class="main_sinfo">Sending with Enter for 1 Coins</div>
            <div class="main_symbol">0/500</div>
        </div>
    </div>
...
<footer>
    ...
    <div role="contentinfo">
        <div class="ft_copy">
            &copy; <strong>{site_name}</strong> {{ $design->link() }}
    ...
    {if isDebug()}
        <div class="ft">
            <p>
                <small>{{ $design->stat() }}</small>
            </p>
        </div>
        <p class="small darkred">
            {lang 'WARNING: Your site is in development mode! You can change the mode'} <a
                    href="{{ $design->url(PH7_ADMIN_MOD,'tool','envmode') }}"
                    title="{lang 'Change the Environment Mode'}" class="darkred">{lang 'here'}</a>
        </p>
    {/if}
</footer>
...
{{ $design->externalJsFile(PH7_URL_STATIC . PH7_JS . 'jquery/jquery-ui.js') }
} {* UI must be the last here, otherwise the jQueryUI buttons won't work *}
